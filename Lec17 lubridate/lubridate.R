...
# EXERCISE: As it is, the dates are simply character strings.  Convert the date
# variable from character strings to date/time objects and plot a time series of
# the avg price of bitcoins relative to USD vs date.  What is the overall trend?

# We convert them to POSIXct objects i.e. date/time objects using the ymd()
...
# week is there the most trading of bitcoins?
...
# Compare these two time intervals
auckland
jsm


#---------------------------------------------------------------
# Set Functions (Works for generic R objects)
#---------------------------------------------------------------
# setdiff(a, b) asks what elements are in a, but not in b:
setdiff(c(1,2,3), c(1,2,5))
...
#
# Hint: nested ifelse statements and the following which is similar to %within%
# but for individual elements, not intervals
c(3,5) %in% c(1,2,3)
...
mutate(month=month(Date),
  ...
  ggplot(data=., aes(x=Date, y=Avg)) + geom_point(aes(col=Season)) +
  xlab("Date") + ylab("Bitcoin price vs US Dollar") + geom_smooth()